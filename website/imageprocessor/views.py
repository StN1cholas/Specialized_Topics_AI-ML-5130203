...
try:
    ...
    def predict_image(image):
        mock_probabilities = np.random.rand(
            len(classes))  # Генерируем случайные вероятности
        mock_probabilities = mock_probabilities / mock_probabilities.sum()  # Нормализуем их
        mock_predicted_class = np.argmax(
            mock_probabilities)  # Находим класс с максимальной вероятностью
        return mock_probabilities, classes[mock_predicted_class]
else:
